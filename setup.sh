...
if command -v python3 >/dev/null 2>&1 && [ -x "$AUTO_DIR/bootstrap_envs.sh" ]; then
  echo "[1/6] Preparing Python environments (venv + docs_venv)..."
  "$AUTO_DIR/bootstrap_envs.sh"
else
  echo "[1/6] Skipping Python environment setup (python3 or bootstrap script not available)."
fi
...
if [ -f "$BACKEND_DIR/package.json" ]; then
  ...
  if [ ! -f "$BACKEND_ENV_FILE" ] && [ -f "$BACKEND_ENV_TEMPLATE" ]; then
    ...
    echo "[3/6] Backend .env already exists or template is missing. Skipping .env creation."
  fi

  echo "[4/6] Initializing backend database..."
...
if [ -f "$ADMIN_FRONTEND_DIR/package.json" ]; then
  echo "[6/6] Installing admin frontend dependencies..."
  (cd "$ADMIN_FRONTEND_DIR" && npm install)
else
  echo "[6/6] Admin frontend package not found. Skipping admin frontend npm install."
fi
